...
from utils.ptz_utils import get_zoom_focus, calc_mid, zoom_at, center_auto_zoom_at
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            if len(outputs) > 0:
                ...
                # print(outputs[0])
                # break
                # x1, y1, x2, y2, idx = outputs[0]
                # mid_x, mid_y = (x1+x2)/2, (y1+y2)/2
                # orig = img0
                # img0 = self.zoom_at(img0, (mid_x, mid_y), zoom=1.7)
                boxes = []
                for (x1, y1, x2, y2) in bbox_xyxy:
                    boxes.append((x1, y1))
                    boxes.append((x2, y2))
                img0 = center_auto_zoom_at(img0, boxes)
                # break
            ...
            if self.args.save_txt:
                with open(self.args.save_txt + str(idx_frame).zfill(4) + '.txt', 'a') as f:
                    for i in range(len(outputs)):
                        ...
                        # print(x1, x2, y1, y2, idx, self.args.save_txt + str(idx_frame).zfill(4) + '.txt')
            idx_frame += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default=f'output/{start_time}', help='output folder')  # output folder
    parser.add_argument("--frame_interval", type=int, default=2)
    parser.add_argument('--fourcc', type=str, default='mp4v', help='output video codec (verify ffmpeg support)')
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--save_txt', default=f'output/{start_time}/predict/', help='boxes')
